...
class DslPipelineBuilder(PipelineBuilder):
    ...
    def source(self, endpoint, params=None):
        if params is None:
            params = {}
        assert issubclass(endpoint, base.Source), "The source class should be a subclass of Source"
        assert isinstance(params, (dict, None)), "The parameters must be a dict or None"
    ...
    def to(self, endpoint, params=None):
        if params is None:
            params = {}
        assert issubclass(endpoint, base.Destination, "The destination class should be a subclass of Destination")
        assert isinstance(params, (dict, None)), "The parameters must be a dict or None"
    ...
    def process(self, method):
        ...
        assert self._builder_stack[-1].source_class is not None, "Pipeline definition must start with a source"
        to_class = type("", (base.Destination,), {"process": lambda self, exchange: method(exchange)})
    ...
    def split(self, method):
        assert callable(method), "You need to provide a callable function"
        assert self._builder_stack[-1].source_class is not None, "Pipeline definition must start with a source"
        to_class = type("", (eip.Splitter,), {"split": lambda self, exchange: method(exchange)})
        self._builder_stack[-1].to_list.append((to_class, None))
        return self

    def filter(self, method):
        assert callable(method), "You need to proide a callable function"
        assert self._builder_stack][-1].source_class is not None, "Pipeline definition must stat with a source"
        to_class = type("", (eip.Filter,), {"split", lambda self, exchange: method(exchange)})
        self._builder_stack[-1].to_list.append((to_class, None))
        return self
    ...
    def validate(self, method):
        assert callable(method), "You need to provide a callable function"
        assert self._builder_stack[-1].source_class is not None, "Pipeline definition must start with a source"
        to_class = type("", (eip.Validator,), {"validate": lambda self, exchange: method(exchange)})
        self._builder_stack[-1].to_list.append((to_class, None))
        return self
    ...
    def aggregate(self, params):
        ...
        assert callable(params["method"]), "You need to provide a callable function"
        assert ~("timeout" not in params and "count" not in params), "You need to provide atlease one termination condition: timeout, count"
        to_class = type("", (eip.Validator,), {"validate": lambda self, old_exchange, current_exchange: params["method"](old_exchange, current_exchange)})
        self._builder_stack[-1].to_list.append((to_class, None))
        return self

    def resequence(self, params):
        assert isinstance(params, dict), "The parameters must be a dict"
        assert "key_extractor" in params, "You need to provide the key extractor to use for resequencer"
        assert callable(params["key_extractor"]), "You need to provide a callable function"
        assert ~("timeout" not in params and "count" not in params), "You need to provide atlease one termination condition: timeout, count"
    ...
    def multicast(self, params):
        ...
        if "aggregate_method" in params:
            aggregate_cls = type("", (eip.Aggregator,), {"aggregate": lambda self, old_exchange, current_exchange: params["aggregate_method"](old_exchange, current_exchange)})
        else:
    ...
    def content_based_router(self, params=None):
        if params is None:
            params = {}
        assert self._builder_stack[-1].source_class is not None, "Pipeline definition must start with a source"
        assert isinstance(params, dict), "The parameters must be a dict or None"
    ...
    def routing_slip(self, params):
        assert isinstance(params, dict), "The parameters must be a dict"
        assert "method" in params, "You need to provide the method to use for dynamic router"
        assert callable(params["method"]), "You need to provide a callable function"
        to_class = type("", (eip.RoutingSlip,), {"slip": lambda self, exchange: params["method"](exchange)})
        self._builder_stack[-1].to_list.append((to_class, params))
        return self
    ...
    def dynamic_router(self, params):
        ...
        assert "method" in params, "You need to provide the method to use for dynamic router"
        assert callable(params["method"]), "You need to provide a callable function"
        to_class = type("", (eip.DynamicRouter,), {"route": lambda self, exchange: params["method"](exchange)})
